...
class NSFPRawMLP(nn.Module):
    ...
    def _make_model(self) -> torch.nn.ModuleList:
        ...
        if self.layer_size <= 1:
            nn_layers.append(
                torch.nn.Sequential(torch.nn.Linear(self.input_dim, self.output_dim))
            )
            return nn_layers

        nn_layers.append(
            torch.nn.Sequential(torch.nn.Linear(self.input_dim, self.latent_dim))
        )
        nn_layers.append(self._get_activation_fn())
        for _ in range(self.layer_size - 1):
            nn_layers.append(
                torch.nn.Sequential(torch.nn.Linear(self.latent_dim, self.latent_dim))
            )
            nn_layers.append(self._get_activation_fn())
